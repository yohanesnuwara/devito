...
class AcousticWaveSolver(object):
    ...
    def op_fwd(self, save=None):
        """Cached operator for ForwardOperator runs with buffered wavefield"""
    ...
    def op_grad(self, save=True):
        ...
        return JacobianAdjointOperator(self.model, save=save, geometry=self.geometry,
                                kernel=self.kernel, space_order=self.space_order,
                                **self._kwargs)

    @memoized_meth
    def op_jacobian_forward(self):
        """Cached operator for JacobianForward runs"""
        return JacobianForwardOperator(self.model, save=None, geometry=self.geometry,
                            kernel=self.kernel, space_order=self.space_order,
                            **self._kwargs)
